...
list_years_acidentes <- list_years_acidentes[list_years_acidentes %nlike% "causa"]

base_id_veiculo <- lapply(seq_along(list_years_acidentes),function(i){
...
vitima_all <- lapply(seq_along(list_years_acidentes),function(i){ # i = 1
  ...
  # read
  tmp_file <- data.table::fread(list_years_acidentes[i],encoding = "Latin-1")
  ...
  # summary
  tmp_file <- tmp_file[,.N,by = .(br,km,tipo_vitima,ano,estado_fisico)]

  return(tmp_file)
}) %>% data.table::rbindlist()


readr::write_rds(vitima_all,"data/vitima_all.rds",compress = "gz")
